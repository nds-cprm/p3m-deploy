<?xml version="1.0" encoding="UTF-8"?>
<sld:StyledLayerDescriptor xmlns="http://www.opengis.net/sld" xmlns:sld="http://www.opengis.net/sld"
    xmlns:gml="http://www.opengis.net/gml" xmlns:ogc="http://www.opengis.net/ogc" version="1.0.0">
   <sld:NamedLayer>
    <sld:Name>Terminais_portuários — Terminais portuários.shp</sld:Name>
    <UserStyle>
      <sld:Name>Terminais_portuários — Terminais portuários.shp</sld:Name>
      <sld:FeatureTypeStyle>
        <sld:Rule>
          <sld:Name>Single symbol</sld:Name>
          <PointSymbolizer>
            <Graphic>
              <Mark>
                <WellKnownName>ttf://Material Symbols Outlined Regular#0Xe584</WellKnownName>
                <Fill>
                  <CssParameter name="fill">#c6c6c6</CssParameter>
                </Fill>
                <Stroke />
              </Mark>
              <Size>50</Size>
            </Graphic>
          </PointSymbolizer>
        </sld:Rule>
      </sld:FeatureTypeStyle>
    </UserStyle>
  </sld:NamedLayer>
</sld:StyledLayerDescriptor>