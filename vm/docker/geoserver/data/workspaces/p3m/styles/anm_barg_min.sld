<?xml version="1.0" encoding="UTF-8"?>
<sld:StyledLayerDescriptor xmlns="http://www.opengis.net/sld" xmlns:sld="http://www.opengis.net/sld"
    xmlns:gml="http://www.opengis.net/gml" xmlns:ogc="http://www.opengis.net/ogc" version="1.0.0">
   <sld:NamedLayer>
    <sld:Name>Ibge - Barragens de mineração</sld:Name>
    <UserStyle>
      <sld:Name>ANM - Barragens de mineração</sld:Name>
      <sld:FeatureTypeStyle>
        <sld:Rule>
          <sld:Name>Barragem de mineração</sld:Name>
          <PointSymbolizer>
            <Graphic>
              <Mark>
                <WellKnownName>ttf://Material Symbols Outlined Regular#0Xf6e7</WellKnownName>
                <Fill>
                  <CssParameter name="fill">#c0c0c0</CssParameter>
                </Fill>
                <Stroke />
              </Mark>
              <Size>25</Size>
            </Graphic>
          </PointSymbolizer>
        </sld:Rule>
      </sld:FeatureTypeStyle>
    </UserStyle>
  </sld:NamedLayer>
</sld:StyledLayerDescriptor>